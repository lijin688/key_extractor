...
import os
import jieba
from comentropy.new_word import get_new_words
from . import util
from conf.config import filename

def load_new_word(filename):
    for nw in get_new_words(filename):
        jieba.add_word(nw)

load_new_word(filename)
...
class WordSegmentation(object):
    ...
    def segment_words(self, sentences, lower=True, use_stop_words=True, use_speech_tags_filter=False):
...
class Segmentation(object):
    ...
    def segment(self, text, lower=False):
        ...
        words_no_filter = self.ws.segment_words(sentences=sentences,
                                                    lower=lower,
                                                    use_stop_words=False,
                                                    use_speech_tags_filter=False)
        words_no_stop_words = self.ws.segment_words(sentences=sentences,
                                                        lower=lower,
                                                        use_stop_words=True,
                                                        use_speech_tags_filter=False)

        words_all_filters = self.ws.segment_words(sentences=sentences,
                                                      lower=lower,
                                                      use_stop_words=True,
